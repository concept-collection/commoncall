...
// We publish every event to all relays and subscribe on all relays, so each
// event can arrive several times. Call semantics (request/accept) must fire
// exactly once, so remember recently seen event ids and drop repeats.
const SEEN_CAP = 1000
...
export class Nostr {
  ...
  private connect(url: string) {
    ...
    ws.onmessage = ev => {
      ...
      if (!sub || !event || typeof event.content !== 'string') return
      if (event.id) {
        ...
        if (this.seen.size > SEEN_CAP) {
          for (const id of this.seen) {
            this.seen.delete(id)
            if (this.seen.size <= SEEN_CAP / 2) break
          }
        }
      }
  ...
  /** Subscribe to a topic. Handler fires once per incoming event. */
...
/** Topic everyone on the page announces on / listens to for presence. */
export const rootTopic = (roomId: string): Promise<string> =>
  sha256Hex(`commoncall:${roomId}`)

/** Per-peer topic used to deliver call requests + WebRTC signaling to a specific peer. */
